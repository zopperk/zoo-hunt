...
## In progress
- [ ] **E2E on the live site in Chrome at a mobile viewport** — welcome ✅, join/create team ✅, clues list ✅; next: clue detail → snap → upload → submitted → scoreboard → profile → map, then HQ review/approve → live score update.
...
- [ ] **Admin at `/admin`** (keep `/hq` redirecting) with password **zaid-is-awesome** (secret set remotely ✅ + .dev.vars ✅)
...
## Waiting on user
- [ ] CI API token needs **D1: Edit** + **Workers R2 Storage: Edit** scopes (deploy step currently fails with 7403). Until then I deploy from local wrangler.
